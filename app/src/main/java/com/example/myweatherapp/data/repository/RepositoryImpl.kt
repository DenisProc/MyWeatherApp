...
import com.example.myweatherapp.cons.YANDEX_MY_KEY
...
class RepositoryImpl : WeatherRepository {
    ...
    override suspend fun getWeather(city: City): Response<WeatherDTO> {
        return RetrofitInstance.api.getWeather(YANDEX_MY_KEY, city.lat, city.lon)
    }

    override fun getCityForDetails(): Weather? {
        return weather
    }

    override fun saveCityForDetails(save: Weather) {
        weather = save
    }

    override fun getWorldWeatherList(): List<Weather> {
        return worldCityList
    }

    override fun saveCityWeather(weatherForSave: Weather) {
        weather = weatherForSave
    }

    override fun getRussianWeatherList(): List<Weather> {
        return russianCityList
    }

    override var worldCityList: List<Weather> = listOf(
 ...
 */
    ...
    override var russianCityList: List<Weather> = listOf(
